...
DEFAULT_ENTITY_TYPES = ["Organism", "Anatomical Structure", "Manufactured Object", "Substance", "Conceptual Entity"]
...
async def extract_graph(
    ...
    results = await derive_from_rows( # run run_strategy row by row
        text_units,
        run_strategy,
        callbacks,
...
def _merge_relationships(relationship_dfs) -> pd.DataFrame:
    ...
    if all_relationships.empty:
        # Return an empty DataFrame with the expected columns
        return pd.DataFrame(columns=["source", "target", "description", "source_id", "weight", "sum"])
        print("All entities is empty")
